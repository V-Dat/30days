...
function Day8() {
  ...
  let indexOfCurrentSong; let nextRandomSong
...
useEffect(() => {
  ...
  const handleAudioEnded = () => {if(document.querySelector('.fa-redo').classList.contains('active')){
...
/// xử lý click song ở list => play
  const handleClickSong = useMemo( () => ({e , music}) => {
  ...
  const handleControlMusic = useMemo( () => (e) => {
  ...
  if(e.target.classList.contains('icon-play')){
    ...
    if(indexOfCurrentSong == undefined){
      ...
      document.querySelector('.audio').play()}       // tai sao chỗ này không sử dụng được elementAudio
  } 

  // xử lý repeat
  if(e.target.classList.contains('fa-redo')){
    e.target.classList.toggle('active');
    if( indexOfCurrentSong == undefined){e.target.click()}
  ...
  if(e.target.classList.contains('fa-random')){
    e.target.classList.toggle('active');
    if(indexOfCurrentSong == undefined ){ e.target.click() }else{
  ...
  if(e.target.classList.contains('fa-step-forward')){
      ...
      if(document.querySelector('.fa-random').classList.contains('active')){
        setAudioCurrent(() => {
          indexOfCurrentSong = nextRandomSong = Math.floor(Math.random()*musics.length)
          // nextRandomSong = Math.floor(Math.random()*musics.length)
          console.log('nextRandomSong', nextRandomSong)
      ...
      )}else if(document.querySelector('.fa-redo').classList.contains('active')){
          elementAudio.currentTime = 0;
      }
      else {
        if(indexOfCurrentSong < musics.length -1){
          indexOfCurrentSong++
        }else{indexOfCurrentSong = 0}
        setAudioCurrent( () => musics[indexOfCurrentSong]);
      }
  ...
  if(e.target.classList.contains('fa-step-backward')){
    ...
    if(document.querySelector('.fa-random').classList.contains('active')){
      ...
      indexOfCurrentSong--
    }else{indexOfCurrentSong = musics.length - 1}
    setAudioCurrent( () => musics[indexOfCurrentSong]);  
    }
      ...
      <div className="row">
        <div className="col l-12 m-12 c-12">
          <div className="music-player player">
            ...
            <div className="player__title">{audioCurrent.name == null ? 'Tên Bài Hát' : audioCurrent.name }</div>
            <div className="player__image">
              <img
                className="player__image__url"
                alt="Tên bài hát"

                src={process.env.PUBLIC_URL + `${audioCurrent.image}` == 'undefined' ? process.env.PUBLIC_URL + "/img/day8/default.jpg" : process.env.PUBLIC_URL + `${audioCurrent.image}`}
              />
            </div>
            <div className="player__control btn" onClick={(e) => handleControlMusic(e)}>
            ...
            <audio id='audio' className="audio" src={audioCurrent.mp3} autoPlay={true} ></audio>
            
            <div className="songs">

            {musics.map(music => (
            <div key= {music.mp3} className="song" onClick={ (e) => handleClickSong( {e, music}) }>
