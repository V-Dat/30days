...
export default function Day7() {
  ...
  const handleDelJob = ({ e, eJob }) => {
    ...
    if (
      e.target.previousElementSibling.previousElementSibling.hasAttribute(
        "checked"
      )
    ) {
      //  trường hợp nếu input checked bị xóa
      setCountLi(() => {
        const countLiChecked =
  ...
  return (
    <div className="day7 background-color">
            ...
            <div className="todo">
              ...
              <div className="todo__add">
                <input
                  ...
                  type="text"
                  value={job}
                  onChange={(e) => handleInputJob(e)}
                  onKeyDown={(e) => handleEnter(e)}
                />
                <button className="todo-add__button" onClick={handleAddJobs}>
                  {" "}
              ...
              <ol className="todo__lists">
                {jobs.map((eJob) => (
                  <li key={Math.random() * 0.193} className="todo__list">
                    <input
                      type="checkbox"
                      checked={checked.includes(eJob)}
                      onChange={() => handleCheckBox(eJob)}
                    />
                    <div
                    ...
                    >
                      ...
                      <div>{eJob || "---Hãy nhập todo list của bạn!---"}</div>
                    </div>
                    <button
                      className="btn-del-job"
                      onClick={(e) => handleDelJob({ e, eJob })}
                    >
                      X
                    </button>
                  </li>
                ...
                <div className="todo__clearAll">
                  <div>
                    {countLi === 0
                      ? `No task pending !`
                      : `You have ${countLi} tasks pending`}
                  </div>
